...
llm_instance = None
llm_instance_for_classifier = None
neo4j_driver = None
...
def initialize_classifier():
    ...
    global bedrock_runtime_client, classification_chain, llm_instance_for_classifier
    ...
    # Initialize LLM
    if llm_instance_for_classifier is None:
        claude_model_kwargs = {
        ...
        llm_instance_for_classifier = ChatBedrock(
...
*Note: This is a fallback analysis. RAG system temporarily unavailable.*"""
        ...
        monthly_savings_potential = (client_data["income"] / 12) - client_data["income"] * 0.6 if client_data["income"] else 0
...
*Note: This is a fallback analysis. RAG system temporarily unavailable.*"""
        ...
        investable_amount = max(0, client_data["savings"] - (client_data["income"] * 0.6 * 6 if client_data["income"] else 0))  # Keep 6-month emergency fund
...
**Recommendations:**
- Maintain 6-month emergency fund: Rs. {client_data["income"] * 0.6* 6 if client_data["income"] else 0 :,.0f}
...
def calculate_debt_to_income(client_data):
    monthly_income = client_data['income']/12
    total_debt = client_data['debt']
    monthly_debt_payment = total_debt*0.03
    if monthly_income>0:
        debt_to_income = (monthly_debt_payment/monthly_income)*100
        return f"{debt_to_income:.1f}"
    else:
        return "N/A"
...
def analyze_client(client_name, query):
    ...
    if rag_response and rag_response.get('success', False):
        ...
        results = {
            ...
            "key_factors": results_data.get('key_factors', "Key factors not available")
        }
